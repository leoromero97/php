		...
		<div class="content">
			...
			<form class="form-inline" method="get">
				<div class="form-group">
					<input name="buscar" class="form-control search mb-20" placeholder="Ingrese el nombre de la materia">
			...
			<table class="table table-striped table-hover">
				...
				<?php
				if($buscar){
					$sql = mysqli_query($con, "SELECT * FROM materias WHERE materia LIKE '%$buscar%' ORDER BY id ASC");
				}else{
					$sql = mysqli_query($con, "SELECT * FROM materias ORDER BY id ASC");
				}
				if(mysqli_num_rows($sql) == 0){
					...
					while($row = mysqli_fetch_assoc($sql)){
						echo '
						<tr>
							<td><a href="profileMateria.php?id='.$row['id'].'"><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>'.$row['id'].'</a></td>
							<td>'.$row['idcurso'].'</td>
							<td>'.$row['materia'].'</td>
						</tr>';
						$no++;
